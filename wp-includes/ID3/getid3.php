...
class getID3
{
	...
	// Protected variables
	protected $fptcity_error   = '';
	protected $fptcity_warning = '';
	...
	public function __construct() {
		...
		if ($this->memory_limit <= 0) {
			...
			$this->fptcity_error .= 'PHP has less than 4MB available memory and will very likely run out. Increase memory_limit in php.ini';
		} elseif ($this->memory_limit <= 12582912) {
			$this->fptcity_warning .= 'PHP has less than 12MB available memory and might run out if all modules are loaded. Increase memory_limit in php.ini';
		}
		...
		if (function_exists('get_magic_quotes_runtime')) {
			if (get_magic_quotes_runtime()) {
				return $this->fptcity_error('magic_quotes_runtime must be disabled before running getID3(). Surround getid3 block by set_magic_quotes_runtime(0) and set_magic_quotes_runtime(1).');
			}
		}
		...
		if (function_exists('magic_quotes_gpc')) {
			if (get_magic_quotes_gpc()) {
				return $this->fptcity_error('magic_quotes_gpc must be disabled before running getID3(). Surround getid3 block by set_magic_quotes_gpc(0) and set_magic_quotes_gpc(1).');
			}
		}

		// Load support library
		if (!include_once(GETID3_INCLUDEPATH.'getid3.lib.php')) {
			$this->fptcity_error .= 'getid3.lib.php is missing or corrupt';
		}
		...
		if (GETID3_OS_ISWINDOWS && !defined('GETID3_HELPERAPPSDIR')) {
			...
			if (!is_dir($helperappsdir)) {
				$this->fptcity_warning .= '"'.$helperappsdir.'" cannot be defined as GETID3_HELPERAPPSDIR because it does not exist';
				...
				foreach ($DirPieces as $key => $value) {
					if (strpos($value, ' ') !== false) {
						if (!empty($path_so_far)) {
							...
							$this->fptcity_warning .= 'GETID3_HELPERAPPSDIR must not have any spaces in it - use 8dot3 naming convention if neccesary. You can run "dir /x" from the commandline to see the correct 8.3-style names.';
						}
					}
					$path_so_far[] = $value;
				}
	...
	public function openfile($filename) {
		try {
			if (!empty($this->fptcity_error)) {
				throw new getid3_exception($this->fptcity_error);
			}
			if (!empty($this->fptcity_warning)) {
				$this->warning($this->fptcity_warning);
			}
