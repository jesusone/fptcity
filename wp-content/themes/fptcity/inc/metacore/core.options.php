...
class ZoCoreControl
{
    ...
    function Scripts()
    {
        ...
        wp_register_style('chosen.jquery', get_template_directory_uri() . '/inc/metacore/assets/css/chosen.css');
        ...
        wp_register_script('chosen.jquery', get_template_directory_uri() . '/inc/metacore/assets/js/chosen.jquery.js');
        ...
        if(isset($params['type'])){
            switch ($params['type']){
                ...
                case 'social':
                    ...
                    break;
                /* LN Muti select postyle */
                case 'selectpostype':
                    return $this->selectpostype($params);
                    break;    
            }
        }
    }
        ...
        <div class="images-field clearfix">
            ...
            foreach ($selected as $value):
                ...
                <li class="items" data-id="<?php echo esc_attr($value); ?>" style="background-image:url(<?php echo esc_url($attachment_image[0]);?>);background-size:cover;">
                    <i class="edit dashicons dashicons-plus-alt" title="<?php _e('Replace Image', 'fptcity'); ?>"></i>
                    <i class="remove dashicons dashicons-dismiss" title="<?php _e('Remove Image', 'fptcity'); ?>"></i>
                </li>
            <?php endif; endforeach; ?>
            <?php if($params['field'] != 'single'): ?>
                <li class="items" data-id=""><i class="add dashicons dashicons-plus-alt" title="<?php _e('Add Image', 'fptcity'); ?>"></i></li>
    ...
    private function social($params){
		...
		<table class="wp-list-table widefat fixed pages zo_social_team">
			<thead>
				<tr>
					<th><?php _e('Title', 'fptcity');?></th>
					<th><?php _e('Link', 'fptcity');?></th>
					<th><?php _e('Icon class', 'fptcity');?></th>
					<th><?php _e('Actions', 'fptcity');?></th>
				</tr>
			</thead>
			<tfoot>
				<tr>
				...
				<tr>
					<td colspan="3">
						<a class="button-primary" id="btn_add_social" href="javascript:void(0);"><?php _e('Add new', 'fptcity');?></a>
					</td>
				</tr>
			</tfoot>
			<tbody  id="the-list">
				<?php
					if(count($zo_social_team)):
						...
						<tr>
							...
							<td><a title="Click to edit this item" href="javascript:void(0)" class="zo_social"><?php _e('Edit', 'fptcity')?></a>|<a class="zo_social_remove" href="javascript:void(0)"><?php _e('Remove', 'fptcity')?></a></td>
						</tr>
						<?php endforeach;
					else: ?>
						<tr>
							<td class="zo_no_item" colspan="3"><?php _e('No item!', 'fptcity');?></td>
						</tr>
					<?php endif;
				?>
			</tbody>
    ...
    /*Add  function selectpostype */
    private function selectpostype($params){
        ob_start();
        wp_enqueue_style('chosen.jquery');
        wp_enqueue_script('chosen.jquery');
        wp_localize_script( 'chosen.jquery', 'ajax_object',
            array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) );
        
        
        /*Get All Postyle*/
        $args = array('post_type'=>'investor');
...
?>    
        <div class="zo-selectpostype">
        <select class="zo-chosen"  multiple="true" style="width:400px;">
        <?php  foreach( $myposts as $post ) :    setup_postdata($post); ?>
            <option value="<?php echo esc_attr($post->ID); ?>" <?php if(in_array($post->ID, $selected)){ echo 'selected="selected"'; } ?>><?php echo esc_attr($post->post_title); ?></option> 
        <?php endforeach; ?>
        </select>
        <input class="zo_select_postype" type="hidden" name="<?php echo esc_attr($params['id']); ?>" id="<?php echo esc_attr($params['id']); ?>" value="<?php echo implode(",", $selected); ?>"/>
    </div>
        <?php
         return ob_get_clean();
